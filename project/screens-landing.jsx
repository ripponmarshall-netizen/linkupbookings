...
const Landing = ({ onCta, onSignIn }) => {
  return (
    <div style={{ background: 'var(--paper)', minHeight: 720, color: 'var(--ink)' }}>
      {/* nav */}
      <div style={{
        ...
        <Logo size={18} />
        <div style={{ display: 'flex', gap: 32, fontSize: 13.5, color: 'var(--ink-2)' }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          <button className="btn btn-primary btn-sm" onClick={onCta}>Start free trial</button>
        </div>
      </div>

      {/* hero */}
      <div style={{ padding: '88px 56px 64px', maxWidth: 1280, margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1.05fr 1fr', gap: 64, alignItems: 'center' }}>
          <div>
            ...
            <h1 className="serif" style={{
              fontSize: 92, lineHeight: 0.95, margin: '0 0 24px',
          ...
          {/* hero illustration — stylized calendar card */}
          <LandingHero />
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 32, opacity: 0.65 }}>
      ...
      {/* features */}
      <div style={{ padding: '96px 56px', maxWidth: 1280, margin: '0 auto' }}>
        <div style={{ maxWidth: 620, marginBottom: 56 }}>
          <div className="label" style={{ marginBottom: 16 }}>What you get</div>
          <h2 className="serif" style={{ fontSize: 56, margin: 0, lineHeight: 1.02, letterSpacing: '-0.015em', fontWeight: 400 }}>
      ...
      {/* testimonial */}
      <div style={{ padding: '96px 56px', maxWidth: 1080, margin: '0 auto', textAlign: 'center' }}>
        <div style={{ fontSize: 36, color: 'var(--terracotta)', marginBottom: 16, fontFamily: 'var(--serif)' }}>“</div>
        <p className="serif" style={{ fontSize: 36, lineHeight: 1.25, margin: '0 0 28px', letterSpacing: '-0.01em' }}>
      ...
      <div style={{ background: 'var(--ink)', color: '#dccfb6', padding: '48px 56px 36px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', maxWidth: 1280, margin: '0 auto', alignItems: 'flex-start' }}>
          ...
          <div style={{ display: 'flex', gap: 64, fontSize: 13 }}>
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <a>Features</a><a>Pricing</a><a>Roadmap</a>
              </div>
            </div>
            <div>
              <div style={{ color: '#fbf6ec', marginBottom: 12, fontWeight: 500 }}>Help</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <a>Docs</a><a>Contact</a><a>WhatsApp us</a>
              </div>
            </div>
            <div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', marginTop: 36, paddingTop: 20, fontSize: 12, color: '#a89c87', maxWidth: 1280, margin: '36px auto 0', display: 'flex', justifyContent: 'space-between' }}>
...
function FeatureCard({ num, title, body, color, tint, paid }) {
  return (
    <div style={{
      ...
      borderRadius: 'var(--r-lg)', padding: 28, position: 'relative',
      overflow: 'hidden',
    }}>
      <div style={{
        fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--muted)',
        letterSpacing: '0.1em', marginBottom: 20,
      }}>{num} {paid && <span style={{ color: 'var(--terracotta)', marginLeft: 6 }}>· PRO</span>}</div>
      <div style={{
...
function PriceCard({ tier, price, suffix, tagline, features, cta, onCta, featured }) {
  return (
    <div style={{
      background: featured ? 'var(--ink)' : 'var(--card)',
      color: featured ? '#fbf6ec' : 'var(--ink)',
      border: featured ? '1px solid var(--ink)' : '1px solid var(--line)',
      borderRadius: 'var(--r-lg)', padding: 36, position: 'relative',
    }}>
      {featured && (
        <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 4 }}>
        <div className="serif" style={{ fontSize: 32, fontStyle: tier === 'Pro' ? 'italic' : 'normal' }}>{tier}</div>
      </div>
      <div style={{ fontSize: 13, color: featured ? '#a89c87' : 'var(--muted)', marginBottom: 24 }}>{tagline}</div>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginBottom: 28 }}>
        <div className="serif" style={{ fontSize: 64, lineHeight: 1, fontWeight: 400 }}>{price}</div>
        {suffix && <div style={{ fontSize: 14, color: featured ? '#a89c87' : 'var(--muted)' }}>{suffix}</div>}
      </div>
      <button
        className={featured ? 'btn btn-terracotta btn-lg' : 'btn btn-primary btn-lg'}
        onClick={onCta}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {features.map(([f, ok], i) => (
          <div key={i} style={{
            ...
            fontSize: 13.5,
            color: ok ? (featured ? '#fbf6ec' : 'var(--ink-2)') : (featured ? '#7a7062' : 'var(--muted-2)'),
            textDecoration: ok ? 'none' : 'line-through',
            textDecorationColor: featured ? '#7a7062' : 'var(--muted-2)',
          }}>
            <div style={{
              width: 18, height: 18, borderRadius: '50%',
              background: ok ? (featured ? 'var(--terracotta)' : 'var(--forest-soft)') : (featured ? '#3a3a33' : 'var(--paper-2)'),
              color: ok ? (featured ? '#fbf6ec' : 'var(--forest)') : (featured ? '#7a7062' : 'var(--muted-2)'),
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              flexShrink: 0,
            }}>
              {ok ? Icon.check({ width: 11, height: 11 }) : Icon.x({ width: 10, height: 10 })}
            </div>
            <span>{f}</span>
          </div>
        ))}
    ...
    <div style={{
      ...
      {rows.map(([label, free, pro], i) => (
        <div key={i} style={{
          ...
          <div style={{ padding: '14px 20px', borderLeft: '1px solid var(--line)', textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {cell(free)}
          </div>
          <div style={{ padding: '14px 20px', borderLeft: '1px solid var(--line)', textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(196,102,61,0.04)' }}>
            {cell(pro)}
          </div>
        </div>
      ))}
...
function LandingHero() {
  // Stylized "calendar card" — a paper artifact, not literal screenshot.
  return (
      ...
      <div style={{
        ...
        {[
          ...
          <div key={i} style={{
            ...
            borderRadius: 10, marginBottom: 8, background: 'var(--card-warm)',
          }}>
        ...
        {[
          ...
          <div key={i} style={{ padding: '14px 18px', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <span className="chip" style={{
              ...
              background: a.tint, color: a.color, border: 'none',
            }}>confirmed</span>
          </div>
        ))}
      </div>
