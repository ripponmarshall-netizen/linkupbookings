// Waitlist screen — capture demand for booked-out slots, auto-fill cancellations
...
function WaitlistScreen({ isPro, onNav, onUpgrade, onShareLink }) {
  const [autoFill, setAutoFill] = React.useState(true);
  const [tab, setTab] = React.useState('queue'); // queue | rules
  ...
  return (
    ...
    >
      ...
      {tab === 'queue' ? <WaitlistQueue autoFill={autoFill} setAutoFill={setAutoFill} /> : <WaitlistRules />}
    </DashboardShell>
  );
}
...
window.WaitlistScreen = WaitlistScreen;
